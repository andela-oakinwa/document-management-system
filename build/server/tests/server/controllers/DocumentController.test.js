...
var request = _supertest2.default.agent(_Server2.default),
    ...
    publicD = _Test2.default.publicDocument,
    privateD = _Test2.default.privateDocument,
    roleDoc = _Test2.default.roleDocument,
    userDetails = _Test2.default.firstUser,
    ownerDetails = _Test2.default.secondUser,
...
describe('DOCUMENT API', function () {
  before(function (done) {
    _models2.default.Role.bulkCreate([_Test2.default.adminRole, _Test2.default.regularRole]).then(function (roles) {
      _Test2.default.adminUser.roleId = roles[0].id;
      _models2.default.User.create(_Test2.default.adminUser).then(function () {
        request.post('/users/login').send(_Test2.default.adminUser).end(function (err, res1) {
          adminToken = res1.body.token;
          request.post('/users').send(_Test2.default.regularUser).end(function (err, res2) {
            regularUser = res2.body.user;
            regularToken = res2.body.token;
            request.post('/users').send(_Test2.default.regularUser2).end(function (err, res3) {
  ...
  describe('CREATE DOCUMENT POST /documents', function () {
    it('should create a new document', function (done) {
      request.post('/documents').send(publicD).set({ 'x-access-token': regularToken }).end(function (err, res) {
    ...
    it('should return varification failed when token is not supplied', function (done) {
      request.post('/documents').send(publicD).end(function (err, res) {
    ...
    it('should not create document when title is not supplied', function (done) {
      ...
      request.post('/documents').send(invalidDoc).set({ 'x-access-token': adminToken }).end(function (err, res) {
        expect(res.status).to.equal(400);
  ...
  describe('Update Document /documents/:id', function () {
    ...
    it('should update document when user is the owner', function (done) {
      ...
      request.put('/documents/' + createdDoc.id).send(updateDoc).set({ 'x-access-token': regularToken }).end(function (err, res) {
    ...
    it('should allow admin to update document', function (done) {
      updateDoc = { title: 'TIA' };
      request.put('/documents/' + createdDoc.id).send(updateDoc).set({ 'x-access-token': adminToken }).end(function (err, res) {
        expect(res.status).to.equal(200);
        expect(res.body.updatedDocument.title).to.equal(updateDoc.title);
        expect(res.body.updatedDocument.content).to.equal(createdDoc.content);
        done();
    ...
    it('should not update document when user is not the owner', function (done) {
      ...
      request.put('/documents/' + createdDoc.id).send(updateDoc).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
    ...
    it('should not update document when token is not supply', function (done) {
      updateDoc = { content: 'new life, new culture, new community' };
      request.put('/documents/' + createdDoc.id).send(updateDoc).end(function (err, res) {
        expect(res.status).to.equal(400);
        expect(res.body.message).to.equal('Please sign in or register to get a token');
        done();
      });
    ...
    it('should return not found when invalid id is supplied', function (done) {
      updateDoc = { content: 'new life, new culture, new community' };
      request.put('/documents/9999').send(updateDoc).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
  ...
  describe('Delete Document DELETE /documents/:id', function () {
    beforeEach(function (done) {
      request.post('/documents').send(privateD).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
    ...
    it('should allow document\'s owner to delete document', function (done) {
      request.delete('/documents/' + document.id).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
        expect(res.status).to.equal(200);
    ...
    it('should allow admin to delete any document', function (done) {
      request.delete('/documents/' + document.id).set({ 'x-access-token': adminToken }).end(function (err, res) {
        expect(res.status).to.equal(200);
        expect(res.body.message).to.equal('This document has been deleted successfully');
        done();
      });
    ...
    it('should not delete document if requester is not the owner or admin', function (done) {
      request.delete('/documents/' + document.id).set({ 'x-access-token': regularToken }).end(function (err, res) {
        expect(res.status).to.equal(401);
        expect(res.body.message).to.equal('You are not permitted to modify this document');
        done();
      });
    ...
    it('should return not found when for invlid id', function (done) {
      request.delete('/documents/999').set({ 'x-access-token': regularToken2 }).end(function (err, res) {
        expect(res.status).to.equal(404);
        expect(res.body.message).to.equal('This document does not exist');
        done();
      });
  ...
  describe('GET document /documents/:id', function () {
    describe('GET document with PRIVATE access', function () {
      before(function (done) {
        request.post('/documents').send(privateD).set({ 'x-access-token': regularToken }).end(function (err, res) {
      ...
      it('should ONLY return the document when the user is the owner', function (done) {
        request.get('/documents/' + privateDocument.id).set({ 'x-access-token': regularToken }).end(function (err, res) {
          expect(res.status).to.equal(200);
      ...
      it('should allow admin to retrieve document with private access level', function (done) {
        request.get('/documents/' + privateDocument.id).set({ 'x-access-token': adminToken }).end(function (err, res) {
          expect(res.status).to.equal(200);
          expect(res.body.message).to.equal('You have successfully retrived this document');
          expect(res.body.document.title).to.equal(privateDocument.title);
          expect(res.body.document.access).to.equal('private');
      ...
      it('should NOT return document when user is not the owner', function (done) {
        request.get('/documents/' + privateDocument.id).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
          expect(res.status).to.equal(401);
    ...
    describe('PUBLIC DOCUMENT', function () {
      before(function (done) {
        request.post('/documents').send(publicD).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
      ...
      it('should return document to all users', function (done) {
        request.get('/documents/' + publicDocument.id).set({ 'x-access-token': regularToken }).end(function (err, res) {
          expect(res.status).to.equal(200);
      ...
      it('should return document not found when invalid id is supplied', function (done) {
        request.get('/documents/99999').set({ 'x-access-token': regularToken }).end(function (err, res) {
          expect(res.status).to.equal(404);
    ...
    describe('ROLE ACCESS DOCUMENT', function () {
      ...
      before(function (done) {
        _models2.default.Role.create(_Test2.default.guestRole1).then(function (guestRole) {
          _Test2.default.secondUser.roleId = guestRole.id;
          request.post('/users').send(_Test2.default.secondUser).end(function (error, response) {
            guestToken = response.body.token;
            request.post('/documents').send(roleD).set({ 'x-access-token': regularToken }).end(function (err, res) {
      ...
      it('should ONLY return document when user has same role as owner', function (done) {
        request.get('/documents/' + roleDocument.id).set({ 'x-access-token': regularToken2 }).end(function (err, res) {
          expect(res.status).to.equal(200);
      ...
      it('should allow admin to view all role level access documents', function (done) {
        request.get('/documents/' + roleDocument.id).set({ 'x-access-token': adminToken }).end(function (err, res) {
          expect(res.status).to.equal(200);
          expect(res.body.document.title).to.equal(roleDocument.title);
          expect(res.body.document.access).to.equal('role');
          expect(res.body.message).to.equal('You have successfully retrived this document');
      ...
      it('should not return document if not of the same role level', function (done) {
        request.get('/documents/' + roleDocument.id).set({ 'x-access-token': guestToken }).end(function (err, res) {
          expect(res.status).to.equal(401);
          expect(res.body.message).to.equal('You are not permitted to view this document');
          done();
        });
  ...
  describe('GET ALL DOCUMENT PAGINATION', function () {
    it('should return all documents to admin user', function (done) {
      request.get('/documents').set({ 'x-access-token': adminToken }).end(function (err, res) {
        expect(res.status).to.equal(200);
    ...
    it('should return all documents with pagination', function (done) {
      request.get('/documents?limit=4&offset=3').set({ 'x-access-token': adminToken }).end(function (err, res) {
        expect(res.status).to.equal(200);
    ...
    it('should return all documents created by a user irrespective of the\n    access level and every other documents with role or puclic access with\n    limit set to 4', function (done) {
      request.get('/documents?limit=4').set({ 'x-access-token': regularToken2 }).end(function (err, res) {
        expect(res.status).to.equal(200);
        res.body.documents.rows.forEach(function (doc) {
  ...
  describe('DOCUMENT SEARCH PAGINATION', function () {
    ...
    it('should return "enter search string" when search query is not supplied', function (done) {
      request.get('/documents/search').set({ 'x-access-token': regularToken2 }).end(function (err, res) {
        expect(res.status).to.equal(400);
  ...
  describe('Fetch all user\'s document', function () {
    it('should return all documents created by a particular user', function (done) {
      request.get('/users/' + regularUser.id + '/documents').set({ 'x-access-token': regularToken }).end(function (err, res) {
        expect(res.status).to.equal(200);
    ...
    it('should return all documents created by a particular user to admin user', function (done) {
      request.get('/users/' + regularUser.id + '/documents').set({ 'x-access-token': adminToken }).end(function (err, res) {
        expect(res.status).to.equal(200);
        expect(res.body.userDocuments.user.id).to.equal(regularUser.id);
        expect(res.body.userDocuments.documents.rows.length).to.be.greaterThan(0);
        res.body.userDocuments.documents.rows.forEach(function (doc) {
    ...
    it('should return all public or role access level\n    documents to a requester user', function (done) {
      request.get('/users/' + regularUser.id + '/documents').set({ 'x-access-token': regularToken2 }).end(function (err, res) {
        expect(res.status).to.equal(200);
        expect(res.body.userDocuments.user.id).to.equal(regularUser.id);
        res.body.userDocuments.documents.rows.forEach(function (doc) {
          expect(doc.access).to.be.oneOf(['role', 'public']);
    ...
    it('should return no document found for invalid id', function (done) {
      request.get('/users/0/documents').set({ 'x-access-token': regularToken }).end(function (err, res) {
        expect(res.status).to.equal(404);
